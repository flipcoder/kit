...
TEST_CASE("Meta","[meta]") {
    ...
    SECTION("serialization") {
        ...
        SECTION("objects") {
            auto m = make_shared<Meta>(Meta::Format::JSON,"{\"one\":1}");
            REQUIRE(m->at<int>("one") == 1);
            m->clear();

            m = make_shared<Meta>();
            m->deserialize(Meta::Format::JSON,"{\"one\":1}");
            REQUIRE(m->at<int>("one") == 1);
        }

        SECTION("arrays") {
            //auto m = make_shared<Meta>(Meta::Format::JSON,"{[1,2,3]}");
        ...
        SECTION("overwriting") {
            auto m = make_shared<Meta>(Meta::Format::JSON,"{\"one\":1}");
            REQUIRE(m->at<int>("one") == 1);
            m->deserialize(Meta::Format::JSON,"{\"two\":2}");
